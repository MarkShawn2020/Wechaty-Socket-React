...
import { ContactItem } from "../../data-sctructure/interface"
...
const CompContactListInfinite = () => {
  ...
  const fetchContactsCount = async () => {
    const { data } = await $http.get(API.wechat.contactsCount, {
      params: { wxid },
    })
  ...
  const _fetchMoreContacts = async () => {
    ...
    console.log({ wxid, skip: skip.current, limit })
    const { data: newContacts } = await $http.get(API.wechat.contacts, {
      params: { wxid, skip: skip.current, limit },
    })
